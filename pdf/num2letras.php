...
*/
function num2letras($num, $fem = false, $dec = true, $uno = 'un', $fin = ' con ', $fin2='') {
...
//            $fin .= ' ' . $matuni[$s];
//      }
//       $fin .= ' (' . $fra . '/1' . str_repeat('0', strlen($fra)) . ')';
       $fin .= sprintf(" %'0-2s/100", $fra);
   }else
      $fin .= ' 00/100';
   if ((int)$ent === 0) return 'Cero ' . $fin . $fin2;
   $tex = '';
   $sub = 0;
   $mils = 0;
   $neutro = false;
   ...
   $tex = $neg . substr($tex, 1) . $fin . $fin2;
   return ucfirst($tex);
//   return strtoupper($tex);
}
function plural($palabra){
    if(strlen($palabra))
        return $palabra . ( stripos('aeiou', $palabra[strlen($palabra)-1])?'s':'es');
    return $palabra;
}
